...
│
├─ 학습 전 voice 끌어올림 (저비용 ROI)
│    → system_prompt_engineering (voice samples + 하이브리드 prompt)
│
└─ "내가 지금 어느 axis 에서 어디까지 왔나?" 메타 결정
     → iteration_loop (EvalGardener — variance / plateau / axis 추천)
```
...
| 메서드 | 한 줄 정의 | 출력 |
|---|---|---|
| [`iteration_loop`](iteration_loop.md) | EvalGardener — eval suite 자가 진화 5-phase 루프. agent (Claude) 가 tactical/strategic 분석 사이에 개입 | 매 라운드 markdown 보고서 + axis pivot 추천 |
| [`system_prompt_engineering`](system_prompt_engineering.md) | 학습 전, 시스템 프롬프트 강화로 voice·grounding 끌어올림. Hybrid (EN rules + KO voice) 가 grounding-heavy 에 결정적 | 행동 테스트 통과율 ↑ (실측 +10-20%) |
